...
public class Day9Part2 {
    ...
    int tailVisitedPosition(List<String> strings) {
        ...
        var ropes = new ArrayList<>(IntStream.rangeClosed(1, ropeLength).mapToObj(i -> new Position(0, 0)).toList());
        visited.add(ropes.get(ropeLength-1));
        ...
        for (String string : strings) {
            ...
            for (int i = 0; i < steps; i++) {
                ropes.set(0, move(ropes.get(0), direction));
                for (int j = 1; j < ropeLength; j++) {
                    ropes.set(j, moveNext(ropes.get(j-1), ropes.get(j)));
                }
                visited.add(ropes.get(ropeLength-1));
            }
        }
    ...
    Position moveNext(Position head, Position tail) {
    ...
    public void unitTest() throws Exception {
        ...
        assertEquals(new Position(2, 0), moveNext(new Position(3, 0), new Position(1, 0)));
        assertEquals(new Position(2, 2), moveNext(new Position(3, 1), new Position(1, 3)));
        assertEquals(new Position(2, 2), moveNext(new Position(2, 3), new Position(1, 1)));
        assertEquals(new Position(2, 2), moveNext(new Position(4, 2), new Position(1, 1)));
        assertEquals(1, tailVisitedPosition(lines));

        var lines2 = Files.readAllLines(Paths.get(ClassLoader.getSystemResource("2022/day9_test2.txt").toURI()), Charset.defaultCharset());
        assertEquals(36, tailVisitedPosition(lines2));
    }
}
