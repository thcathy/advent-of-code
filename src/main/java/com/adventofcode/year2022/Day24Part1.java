...
import static com.adventofcode.year2022.Day24Part1.Direction.*;
...
public class Day24Part1 {
    ...
    int fewestMinutesToReachGoal(Valley valley) {
        ...
        var queue = new PriorityQueue<>(Comparator.comparing(State::costToGoal).thenComparing(State::minutes));
        var blizzardsPerMinute = new HashMap<Integer, List<Blizzard>>();
        var blizzardsPositionPerMinute = new HashMap<Integer, Set<Position>>();
        var visited = new HashSet<String>();

        queue.add(startState);
        blizzardsPerMinute.put(0, valley.blizzards);
        blizzardsPositionPerMinute.put(0, valley.blizzards.stream().map(Blizzard::position).collect(Collectors.toSet()));

        while (true) {
            var state = queue.poll();

            if (state.isGoal()) return state.minutes;
            ...
            for (Direction moveDirection : nextDirections) {
    ...
    static final List<Direction> nextDirections = List.of(SOUTH, EAST, NONE, WEST, NORTH);
    ...
    record State(int minutes, Position position, Valley valley) {
        boolean isGoal() { return valley.endPosition.equals(position); }

        int distanceToGoal() { return distance(valley.endPosition, position); }

        int costToGoal() { return minutes + distanceToGoal(); }
    }
    ...
    record Valley(List<Blizzard> blizzards, int maxX, int maxY, Position startPosition, Position endPosition) {
        public boolean valid(Position position) {
            return position.equals(endPosition) || (position.x > 0 && position.y > 0 && position.x < maxX && position.y < maxY);
        }
    }
    ...
    public void unitTest() throws Exception {
        ...
        assertEquals(18, fewestMinutesToReachGoal(valley));
    }
