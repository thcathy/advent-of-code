...
import static com.adventofcode.year2022.Day19Part2.ResourceType.*;
import static org.junit.Assert.assertEquals;

public class Day19Part2 {
    final static String inputFile = "2022/day19.txt";
    final static int MAX_MINUTE = 32;
    ...
    void run() throws Exception {
        ...
        var result = multipleMaxGeodesCollectedOfFirst3Blueprints(parseBlueprint(lines));
        System.out.println("What do you get if you multiply these numbers together? " + result);
    }

    int multipleMaxGeodesCollectedOfFirst3Blueprints(List<Blueprint> blueprints) {
        return blueprints.parallelStream().limit(3)
                .mapToInt(this::maxGeodesCollected)
                .reduce(1, (a,b) -> a * b);
    }
    ...
    int maxGeodesCollected(Blueprint blueprint) {
        ...
        var v = maxGeodesCollected(firstState, 0, new HashSet<>());
    ...
    int maxGeodesCollected(State state, int maxGeodesCollected, HashSet<String> visitedStateKeys) {
        if (state.minute == MAX_MINUTE) {
            return Math.max(maxGeodesCollected, state.resources.getOrDefault(Geode, 0));
        }
    ...
    class State {
        ...
        String key() {
            return String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
                    minute,
                    resources.getOrDefault(Ore, 0), resources.getOrDefault(Clay, 0),
                    resources.getOrDefault(Obsidian, 0), resources.getOrDefault(Geode, 0),
                    robots.getOrDefault(Ore, 0), robots.getOrDefault(Clay, 0),
                    robots.getOrDefault(Obsidian, 0), robots.getOrDefault(Geode, 0),
    ...
    public void unitTest() throws Exception {
        ...
        assertEquals(56, maxGeodesCollected(blueprint1));
    }
