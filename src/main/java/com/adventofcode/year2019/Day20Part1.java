...
public class Day20Part1 {
    ...
    int minimumStepToCollectAllKeys(List<String> inputs) {
        ...
        var paths = findAllPaths(maze, innerGates, outerGates);
        var routes = new PriorityQueue<Route>(Comparator.comparingInt(r -> r.steps));

        routes.offer(new Route(startPosition, 0));
        var visitedPosition = new HashSet<Position>();

        while (routes.size() > 0) {
            Route route = routes.poll();

            if (Objects.equals(endPosition, route.position)) {
                return route.steps;
            }

            for (Path nextPath : paths.get(route.position)) {
                if (!visitedPosition.add(nextPath.to)) continue;
                var nextRoute = new Route(nextPath.to, route.steps + nextPath.steps);
    ...
    private Map<Position, List<Path>> findAllPaths(char[][] maze, Map<String,Position> outerGates, Map<String,Position> innerGates) {
        ...
        for (var outerEntry : outerGates.entrySet()) {
            if (innerGates.containsKey(outerEntry.getKey())) {
                ...
                pathsFromOuter.add(new Path(outerEntry.getValue(), innerPosition, 1));

                var pathsFromInner = pathsPerPosition.computeIfAbsent(innerPosition, (k) -> new ArrayList<>());
                pathsFromInner.add(new Path(innerPosition, outerEntry.getValue(), 1));
            }
        }
    ...
    static class Route {
        ...
        public Route(Position position, int step) {
    ...
    Optional<Path> findShortestPath(Position from, Position to, char[][] map) {
        ...
        while (positions.size() > 0) {
            ...
            if (Objects.equals(pos, to)) {
                return Optional.of(new Path(from, to, step));
            } else {
    ...
    static class Path {
        ...
        public String toString() {
            return new StringJoiner(", ", Path.class.getSimpleName() + "[", "]")
                    .add("from=" + from).add("to=" + to).add("steps=" + steps).toString();
        }
    }
