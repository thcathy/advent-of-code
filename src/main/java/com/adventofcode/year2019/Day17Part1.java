...
public class Day17Part1 {
    ...
    void run() throws IOException {
        ...
        var scaffold = scaffoldPositions(software.output);
        var result = scaffold.stream()
    ...
    boolean isIntersection(Position position, Set<Position> scaffold) {
        return scaffold.contains(new Position(position.x-1, position.y))
                && scaffold.contains(new Position(position.x+1, position.y))
                && scaffold.contains(new Position(position.x, position.y-1))
                && scaffold.contains(new Position(position.x, position.y+1));
    }
